...
use crate::{admin_session::AdminSession, dbmodels::UserRole, define_api_response};
...
/// Test
#[openapi(tag = "Users", operation_id = "deleteUserRole")]
#[delete("/user/<user_id>/role", data = "<role_id>")]
pub async fn endpoint(user_id: String, role_id: Json<RoleData>, conn: crate::db::DbConn, _session: AdminSession) -> Result<Response, Error> {
    ...
    let _ = conn.run(|c| {
        ...
        diesel::delete(&user_role).execute(c)
    }).await?;
